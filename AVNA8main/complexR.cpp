...
// leadj true or false;  cdigits 0 to 15
void Complex::cSetFormat(bool cm, bool pr, bool ti, bool lj, uint16_t cd)
    {
    comma = cm;
    parens = pr;
    traili = ti;
...
size_t Complex::printTo(Print& p) const
{
    size_t n = 0;
    if (parens)   n += p.print('(');
    n += p.print(re, cdigits);
    if (comma) n += p.print(',');
    else   n += p.print(' ');
    if (leadj)  n += p.print('j');
    n += p.print(im, cdigits);
    if (traili) n += p.print('i');
    if (parens)   n += p.print(')');
    return n;
};
